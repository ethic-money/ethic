...
const INITIAL_STAKE_MULTIPLE = new BN(10).pow(new BN(6)); // 100 ETH -> 100M ETHCS
...
describe('Pool', function () {
  ...
  beforeEach(async function () {
    ...
    this.ethic = await MockToken.new("Ethic Money", "ETHC", 18, {from: ownerAddress, gas: 8000000});
    this.dai = await MockToken.new("Maker DAI", "DAI", 18, {from: ownerAddress, gas: 8000000});
    ...
    await this.pool.set(this.dao.address, this.ethic.address, this.univ2.address);
  });
  ...
  describe('frozen', function () {
    ...
    describe('when claim', function () {
      beforeEach(async function () {
        ...
        await this.ethic.mint(this.pool.address, 1000);
      ...
      describe('simple', function () {
        ...
        it('updates users balances', async function () {
          expect(await this.ethic.balanceOf(userAddress)).to.be.bignumber.equal(new BN(1000));
        ...
        it('updates dao balances', async function () {
          expect(await this.ethic.balanceOf(this.pool.address)).to.be.bignumber.equal(new BN(0));
    ...
    describe('when bond', function () {
      ...
      describe('with reward', function () {
        describe('before bonding', function () {
          ...
          it('updates users balances', async function () {
            expect(await this.ethic.balanceOf(userAddress)).to.be.bignumber.equal(new BN(0));
            expect(await this.pool.balanceOfClaimable(userAddress)).to.be.bignumber.equal(new BN(0));
            expect(await this.pool.balanceOfRewarded(userAddress)).to.be.bignumber.equal(new BN(1000));
          });

          it('updates dao balances', async function () {
            expect(await this.ethic.balanceOf(this.pool.address)).to.be.bignumber.equal(new BN(1000));
            expect(await this.pool.totalClaimable()).to.be.bignumber.equal(new BN(0));
        ...
        describe('multiple with reward first', function () {
          beforeEach(async function () {
            await this.univ2.faucet(userAddress1, 1000);
            await this.ethic.mint(this.pool.address, new BN(1000));
            await this.univ2.approve(this.pool.address, 1000, {from: userAddress1});
            await this.pool.deposit(1000, {from: userAddress1});
          ...
          it('updates dao balances', async function () {
            expect(await this.ethic.balanceOf(this.pool.address)).to.be.bignumber.equal(new BN(2000));
        ...
        describe('multiple without reward first', function () {
          beforeEach(async function () {
            ...
            await incrementEpoch(this.dao);
            await this.ethic.mint(this.pool.address, new BN(1000).mul(INITIAL_STAKE_MULTIPLE));
          ...
          it('updates dao balances', async function () {
            expect(await this.ethic.balanceOf(this.pool.address)).to.be.bignumber.equal(new BN(1000).mul(INITIAL_STAKE_MULTIPLE));
  ...
  describe('when pause', function () {
    ...
    describe('as dao', function () {
      beforeEach(async function () {
        await this.pool.set(mockDao, this.ethic.address, this.univ2.address);
      ...
      describe('claim', function () {
        ...
        it('basic claim check', async function () {
          expect(await this.ethic.balanceOf(userAddress)).to.be.bignumber.equal(new BN(200));
        });
      });
    });
  ...
  describe('when emergency withdraw', function () {
    ...
    describe('as dao', function () {
      beforeEach(async function () {
        ...
        await this.pool.emergencyWithdraw(this.ethic.address, 1000, {from: mockDao});
      });

      it('transfers funds to the dao', async function () {
        expect(await this.univ2.balanceOf(mockDao)).to.be.bignumber.equal(new BN(1000));
        expect(await this.univ2.balanceOf(this.pool.address)).to.be.bignumber.equal(new BN(0));
        expect(await this.ethic.balanceOf(mockDao)).to.be.bignumber.equal(new BN(1000));
        expect(await this.ethic.balanceOf(this.pool.address)).to.be.bignumber.equal(new BN(0));
      });
    });
